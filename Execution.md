...
CREATE TABLE IF NOT EXISTS pillar_digests (
    ...
    edition_date DATE NOT NULL,              -- The date this digest covers

    -- Content (stored as structured JSON)
    content JSONB NOT NULL DEFAULT '{}',
    -- content structure:
    -- {
    --   "header": { "pillar_name": "...", "date": "...", "time": "..." },
    --   "stories": [
    --     {
    --       "story_id": 123,
    --       "headline": "...",
    ...
    --   ],
    --   "quotes": [
    --     { "quote": "...", "person": "...", "source": "...", "story_id": 123 }
    --   ],
    --   "sentiment": {
    --     "overall": "neutral",       -- positive/neutral/negative
    --     "note": "Coverage is mostly factual with..."
    --   },
    --   "introduction": "AI-generated overview paragraph..."
    -- }

    -- Story references (for quick lookups without parsing JSON)
    story_ids INTEGER[] DEFAULT '{}',
    story_count INTEGER DEFAULT 0,

    -- Sentiment (denormalized for quick filtering)
    sentiment TEXT,                          -- 'positive', 'neutral', 'negative'

    -- Export URLs (populated after document generation)
    docx_url TEXT,                           -- GCS URL for Word document
    pdf_url TEXT,                            -- GCS URL for PDF document

    -- Generation metadata
    generation_method TEXT DEFAULT 'scheduled',  -- 'scheduled', 'on_demand', 'manual'
...
CREATE INDEX idx_pillar_digests_date ON pillar_digests(edition_date DESC);
CREATE INDEX idx_pillar_digests_status ON pillar_digests(status) WHERE status = 'active';
CREATE UNIQUE INDEX idx_pillar_digests_unique ON pillar_digests(pillar_id, edition_date, generated_at);

-- Auto-update timestamp trigger
CREATE TRIGGER update_pillar_digests_updated_at
